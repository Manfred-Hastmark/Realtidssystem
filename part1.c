...
int volume(ToneGenerator* self, int change) 
{
	...
	if(self->volume + change > 20 || self->volume + change < 0) return self->volume;
...
int load(BackgroundTask* self, int change)
{
	...
	if(self->backgroundLoopRange + change > 24000 || self->backgroundLoopRange + change < 1000) return self->backgroundLoopRange;
